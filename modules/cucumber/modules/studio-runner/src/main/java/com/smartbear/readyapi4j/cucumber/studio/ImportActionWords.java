...
public class ImportActionWords extends CommandBase {
    ...
    @CommandLine.Option(names = {"-p", "--project"}, description = "a Cucumber Studio project id")
    ...
    public void run() {
        try {
            ...
            else if( studioProject == null ){
                System.err.println("Missing Cucumber Studio Project id");
                return;
            }
    ...
    private void ensureTheOasDefinitionAt() throws IOException {
        ...
        Request request = buildStudioRequest(new Request.Builder().url(studioEndpoint + "projects/" + studioProject + "/actionwords/" + oasActionWordId)
    ...
    private Set<String> getExistingActionWords() throws IOException {
        ...
        System.out.println("Found " + existingWords.size() + " existing ActionWords in Cucmber Studio project");
    ...
    private String addActionWord(String from, String key, boolean hasFreetext, String description) throws IOException {
        ...
        Request request = buildStudioRequest(new Request.Builder().url(studioEndpoint + "projects/" + studioProject + "/actionwords")
        ...
        if (hasFreetext) {
            ...
            request = buildStudioRequest(new Request.Builder().url(studioEndpoint + "projects/" + studioProject + "/actionwords/" + id)
                    .patch(RequestBody.create(content, JSON_MEDIA_TYPE)));
